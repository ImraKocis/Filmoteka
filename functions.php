...
// function add_meta_box_nastavnici_na_predmetu(){
// 	//add_meta_box('filmoteka_korisnici_koji_su_psudili', 'Korisnici koji su posudili ovaj film', 'html_meta_box_korisnici_posudili', 'film');
// }
	
// function html_meta_box_korisnici_posudili($post){
// 	wp_nonce_field("spremi_nastavnike_na_predmetu", "korisnicik_koji_su_posudili_nonce");
// 	$korisnici_ids = get_post_meta($post->ID, 'korisnicik_koji_su_posudili', true);
// 	$korisnici_ids = explode(',', $korisnici_ids);
	
//     $korisnici_all = get_users('role=subscriber');
	
// 	$korisnici_form = '<script type="text/javascript">
...
//    });
//    </script>';
// 	$korisnici_form .= '<select name="korisnici[]" id="korisnici[]" class="js-example-responsive" multiple="multiple">';
// 	foreach ($korisnici_all as $korisnik){
// 		$selected_text = (in_array($korisnik->ID, $korisnici_ids)) ? "selected" : "";
// 		$korisnici_form .= '<option '.$selected_text.' value="'.$korisnik->ID.'">'.$korisnik->display_name.'</option>';
// 	}
// 	$korisnici_form .= "</select>";
// 	echo($korisnici_form);
// }

// function spremi_korisnika($post_id){
//     $is_autosave = wp_is_post_autosave( $post_id );
//     $is_revision = wp_is_post_revision( $post_id );

//     $is_valid_nonce_korisnik = ( isset( $_POST[ 'korisnici' ] ) && wp_verify_nonce(
//         $_POST[ 'korisnici' ], basename( __FILE__ ) ) ) ? 'true' : 'false';
//     if ($is_autosave || $is_revision || !$is_valid_nonce_korisnik) return;
//     if(!empty($_POST['korisnici'])){
// 		$korisnici = implode(",", $_POST[ 'korisnici' ]);
//         update_post_meta($post_id, 'korisnicik_koji_su_posudili',
//         $korisnici);
//     }
//     else delete_post_meta($post_id, 'korisnici');
// }
//add_action('add_meta_boxes', 'add_meta_box_nastavnici_na_predmetu');
//add_action( 'save_post', 'spremi_korisnika' );

//add_action( 'cmb2_admin_init', 'filmoteka_register_datum_metabox' );
...
// 	foreach ($filmovi as $film){
// 		$sFilmUrl = $film->guid;
// 		$sFilmNaziv = $film->post_title;
// 		$sHtml .= '<li><a href="'.$sFilmUrl.'">'.$sFilmNaziv.'</a></li>';
// 	}
// 	$sHtml .= "</ul>";
// 	return $sHtml;
// }

// function daj_nastavnike($slug, $nastavnici_predemta_ids){
// 	$args = array(
// 		'posts_per_page' => -1,
// 		'post_type' => 'nastavnik',
// 		'post_status' => 'publish',
...
// 			$selected_text = (in_array($nastavnik->ID, $nastavnici_predemta_ids)) ? "selected" : "";		
// 			$sHtml .= "<option".$selected_text." value='".$nastavnik->ID."'>".$nastavnik->post_title."</option>";
// 		}  
// 		return $sHtml;
// }
// function daj_sve_nastavnike(){
// 	$args = array(
//         'posts_per_page' => -1,
//         'post_type' => 'nastavnik',
//         'post_status' => 'publish',
...
// 	return $nastavnici;
// }
// function nastavnik_ime_prezime($nastavnici_ids, $nastavnici){
//     $nastavnici_ids = explode(',', $nastavnici_ids);
//     $imeprezime = "";
//     foreach ($nastavnici_ids as $nastavnikId){
//         foreach ($nastavnici as $nastavnik){
//             if($nastavnik->ID == $nastavnikId)
//                 $imeprezime .= $nastavnik->post_title . ", ";
//         }
//     }
//     return substr($imeprezime, 0, -2);
// }
